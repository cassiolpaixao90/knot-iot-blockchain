...
export class WaterFlowController {
  ...
  async getWaterFlowList(): Promise<any> {
    let userRes = await this.zipkinFetch(
      "http://localhost:3000/users/user-by-id/parthghiya"
    );
  ...
  @OnUndefined(404)
  async getWaterFlowById(
    @Param("waterFlowId") waterFlowId: number
  ): Promise<any> {
    return { msg: "This is first Typescript Microservice" };
  }
  ...
  async addUpdateWaterFlow(
    ...
    if (result.id) {
      ...
      let newWaterFlowResult = Object.assign({
        newWaterFlow: newWaterFlow.getClientWaterFlowModel()
      });
  ...
  async getWaterFlowByType(
    @Param("waterFlowType") waterFlowType: string
  ): Promise<any> {
    return { msg: "This is first Typescript Microservice" };
  }
